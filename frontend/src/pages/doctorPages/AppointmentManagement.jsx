...
import { CalendarToday, Search, Close } from '@mui/icons-material';
...
const AppointmentManagement = () => {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md m-6">
      <div className="flex justify-between items-center mb-4">
        <div className="flex space-x-8 text-sm font-semibold text-gray-500">
          {['Today Appointment', 'Upcoming Appointment', 'Previous Appointment', 'Cancel Appointment'].map((tab) => (
            <Button
              key={tab}
              className={activeTab === tab ? '!text-blue-800 !border-b-2 !border-blue-600' : 'text-gray-400'}
        ...
        <div className="flex items-center space-x-4">
          ...
          />
          <Button variant="outlined" color="primary" startIcon={<CalendarToday />} className="!text-sm !font-semibold !poppins" onClick={() => setOpenCustomDateModal(true)}>
            Any Date
          </Button>
          <Button variant="contained" color="primary" className="!text-xs poppins" onClick={() => navigate('/doctor/appointment-time-slot')}>
      ...
      <CancelAppointmentModal open={openCancelAppointmentModal}onClose={() => setOpenCancelAppointmentModal(false)} onProceed={handlePaymentReturn} />
      <PaymentReturnModal open={openPaymentReturnModal} onClose={() => setOpenPaymentReturnModal(false)} onConfirm={handleConfirmCancelAppointment} />
      <CustomDateFilter open={openCustomDateModal} onClose={() => setOpenCustomDateModal(false)} onApply={handleApplyDateFilter} 
        onReset={handleResetDateFilter} />
      <RescheduleAppointmentModal open={openRescheduleModal} onClose={() => setOpenRescheduleModal(false)} appointment={appointmentToReschedule}
        timeSlots={timeSlots} onSave={handleSaveReschedule}/>
    </div>
  );
};
