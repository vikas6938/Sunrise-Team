...
const AppointmentsList = () => {
  ...
  const fetchTodaysAppointments = async () => {
    try {
      ...
      const todaysAppointments = allAppointments.filter(appointment => moment(appointment.appointmentDate).isSame(today, "day"));
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-gray-500 poppins">No Appointments Found for Today</p>
        </div>
      )}
    </div>
  );
