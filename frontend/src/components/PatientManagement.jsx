    ...
    <div className="bg-gray-100 h-full">
      <div className="bg-white p-4 rounded-lg h-full shadow-md">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-semibold ms-2">{activeTab}</h2>
              ...
              {filteredAndSearchedAppointments.length > 0 ? (
                filteredAndSearchedAppointments.map((appointment) => (
                  <tr key={appointment.id} className="border-b">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-4 py-2 rounded-full ${appointmentTypeStyles[appointment.appointmentType]}`}>{appointment.appointmentType}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <button className="text-blue-600" onClick={() => handleViewPatient(appointment.id)}><FaEye /></button>
                    </td>
                  </tr>
                ))
              ) : (
      ...
      {isModalOpen && (
        <PatientDetailsModal open={isModalOpen} handleClose={handleCloseModal} patient={selectedPatient} />
      )}
    </div>
  );
};
