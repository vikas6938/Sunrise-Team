...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    if (!email) {
      validationErrors.email = "Email ID or Phone Number is required.";
    }
    if (!password) {
        ...
        <div className="w-full max-w-md bg-white p-10 rounded-xl shadow-lg">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="relative mb-4">
              ...
              />
              <label htmlFor="password" className="absolute left-3 -top-2.5 px-1 bg-white text-sm font-medium text-[#030229]  transition-all duration-200 peer-focus:-top-2.5 peer-focus:left-3">Password<span className="text-red-500">*</span></label>
              {errors.password && (
            ...
            <button type="submit" className="w-full py-2  bg-[#f6f8fb] text-[#4f4f4f] rounded-xl hover:bg-[#0eabeb] hover:text-white transition duration-200" >Login</button>
          </form>
          <p className="text-center mt-4 text-sm">Don’t have an account?{" "}
            <Link to={"/signup"} className="text-blue-500 hover:underline">Registration</Link>
          </p>
        </div>
      </div>
      <SidePanel />
